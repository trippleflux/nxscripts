            ...
            switch -- $status {
                0 - 3 {
                    SendTargetTheme $target Module::Online ${theme}Body \
                        [list $user $group $idle $path]
                }
                1 - 2 {
                    set total [expr {$total + $speed}]
                    SendTargetTheme $target Module::Online ${theme}Body \
                        [list $user $group $size $speed [file dirname $path] [file tail $path]]
                }
            }
        }
    } else {
...
proc ::Bot::Mod::Online::Users {event target user host channel argv} {
    ...
    if {$usersTotal} {
        SendTargetTheme $target Module::Online ${theme}Foot \
            [list $speedDn $speedUp $speedTotal $usersDn $usersUp $usersIdle $usersTotal $usersMax]
    } else {
