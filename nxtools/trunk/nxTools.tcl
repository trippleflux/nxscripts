...
proc ToolsMain {ArgV} {
    ...
    switch -exact -- $Action {
        {bot} {
            foreach {Action SubAction Param1 Param2 Param3} $args {break}
            set SubAction [string tolower $SubAction]
            if {[string equal "auth" $SubAction] && $ArgLength > 3} {
                set Result [BotAuth $Param1 $Param2]
            } elseif {[string equal "stats" $SubAction] && $ArgLength > 3} {
                ## Check stats type
                set StatsType [string tolower $Param1]
                if {[lsearch -exact {alldn allup daydn dayup monthdn monthup wkdn wkup} $StatsType] == -1} {
                    set StatsType "Stats"
                }

                ## Check section number
                if {![string is digit -strict $Param3] || $Param3 > 9} {
                    set StartIndex 1; set EndIndex "end"
                } else {
                    set StartIndex [incr Param3]; set EndIndex [incr Param3 2]
                }

                ## User or group stats
                if {[string index [set Target $Param2] 0] == "="} {
                    set Target [string range $Target 1 end]
            ...
            } elseif {[string equal "user" $SubAction] && $ArgLength > 2} {
                set Result [BotUserInfo $Param1 $Param2]
            } elseif {[string equal "who" $SubAction]} {
                set Result [BotWho]
            } else {
                iputs "Syntax: SITE BOT AUTH <username> <password>"
                iputs "        SITE BOT STATS <stats type> <username/=group> \[stats section\]"
                iputs "        SITE BOT USER <username> \[credit section\]"
                iputs "        SITE BOT WHO"
            }
        }
        {daystats} {
